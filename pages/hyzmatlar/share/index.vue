...
export default {
  ...
  mounted(){
    ...
    const stop = setInterval(function(){
      const t1 = new Date();
      const t2 = new Date("2022-06-22 23:29:00");
      console.log(t2.getTime())
      var dif = ( t2.getTime() - t1.getTime() ) / 1000;
      dif = Math.round(dif);
      let result = new Date(dif * 1000).toISOString().slice(11, 19);
      if(dif < 1){
        clearInterval(stop);
      }else{
        // let times = result.split(':');
        // console.log(times)
        let hour = result.slice(0,2);
        let minut = result.slice(3,5);
        let secund = result.slice(6,8);
        const span = document.querySelectorAll(".timer .span");
        span[0].textContent = hour;
        span[1].textContent = minut;
        span[2].textContent = secund;
      }
    },1000)
